...
def get_crox_options_chain():
    """Fetch CROX put options chain with minimum 18 days to expiration."""
    util.startLoop()
    ib = IB()
    ...
    try:
        ...
        # Qualifiziere CROX Aktie
        crox = Stock('CROX', 'SMART', 'USD')
        ib.qualifyContracts(crox)
        logger.info(f"🔑 CROX conId: {crox.conId}")
        
        # Hole verfügbare Optionen (SecDef)
        chains = ib.reqSecDefOptParams(crox.symbol, '', crox.secType, crox.conId)
        
        # Finde SMART-Kette
        chain = None
        for c in chains:
            if c.exchange == 'SMART' or c.tradingClass == 'CROX':
                chain = c
                break
        if not chain:
        ...
        if not chain:
            logger.error("❌ Keine Optionen-Kette gefunden")
            return None
        ...
        for exp in selected_exps:
            for strike in strikes:
                contract = Option('CROX', exp, strike, 'P', 'SMART', tradingClass='CROX')
        ...
        out_path = "/home/hermes/crox_options_18days.csv"
